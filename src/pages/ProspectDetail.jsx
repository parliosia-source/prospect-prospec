...
function isValidContactUrl(url, domain) {
  if (!url || typeof url !== "string") return false;
  const trimmed = url.trim();
  // Must be a proper HTTP(S) URL
  if (!/^https?:\/\/.+\..+/i.test(trimmed)) return false;
  // Reject fabricated /contact fallback URLs that are just domain + /contact
  // These are generated by the backend as stubs and are often 404s
  if (domain) {
    const domainNorm = domain.replace(/^www\./, "").toLowerCase();
    const stripped = trimmed.replace(/^https?:\/\/(www\.)?/i, "").replace(/\/+$/, "").toLowerCase();
    if (stripped === `${domainNorm}/contact` || stripped === `${domainNorm}/nous-joindre`) {
      return false; // Likely a fabricated fallback — don't display
    }
  }
  return true;
}
              ...
              return (
                <div key={c.id} className={`flex items-center justify-between p-3 rounded-lg ${isStub ? "bg-amber-50 border border-amber-100" : "bg-slate-50"}`}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isValidContactUrl(c.contactPageUrl, prospect?.domain) && !c.contactPageUrl.includes(c.linkedinUrl || "__none__") && (
