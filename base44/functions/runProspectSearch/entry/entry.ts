...
Deno.serve(async (req) => {
  try {
    ...
    if (campaign.kbOnlyMode) {
      ...
      for (const query of queries) {
        ...
        for (const result of results) {
          ...
          // Filtrer les excluded keywords du tenant
          const excludedKws = tenant.excludedKeywords || [];
          const snippetLower = (prospect.serpSnippet + " " + prospect.companyName).toLowerCase();
          if (excludedKws.some((kw: string) => snippetLower.includes(kw.toLowerCase()))) continue;
    ...
    await base44.asServiceRole.entities.Campaign.update(campaignId, {
      ...
      lastRunDebugSummary: `tenant=${tenant.tenantId} geo=${geographies[0]} queries=${queries.length} brave=${braveRequestCount} inserted=${insertedCount}/${target} scoring=${tenant.scoringMode || "RULES"}`,
    });
